...
/// <summary>Immutable snapshot of the live game for the overlay (EA 0.11 Mono).</summary>
...
/// <summary>Reads a read-only snapshot via SaveGameManager.Current. Null-safe; never throws.</summary>
internal static class GameProbe
{
    public static GameSnapshot Read()
    {
        ...
        try
        {
            ...
            if (gi == null) return s;
            s.HasSave = true;
            s.Day = gi.Day; s.Hour = gi.Hour; s.Minute = gi.Minute;
            s.Money = gi.Money; s.NetWorth = gi.NetWorth; s.Energy = gi.Energy; s.Happiness = gi.Happiness;
        ...
        catch (Exception ex) { UnityEngine.Debug.LogWarning("[BA BOT] probe read failed: " + ex.Message); }
        return s;
    }
