...
internal static class Loc
{
    ...
    private static readonly Dictionary<string, string> Da = new()
    {
        ...
        ["Buys products back up to target when shelves run low (live in Live mode; previews otherwise)."]
            = "Køber varer op til målniveau når hylderne er lave (live i Live-tilstand; ellers forhåndsvisning).",
        ...
        ["OFF (default) = automation only PREVIEWS what it would do (safe). ON = it actually pays taxes, restocks and gives bonuses. Turn on only while watching the game."]
            = "FRA (standard) = automatisering FORHÅNDSVISER kun hvad den ville gøre (sikkert). TIL = den betaler faktisk skat, genopfylder og giver bonusser. Slå kun til mens du holder øje med spillet.",
